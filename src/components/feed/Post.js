...
const Post = ({ userId, tweetContent, tweetFilePath, likesCount , timestamp}) => {
    ...
    <>
      <div id='post' className='grid p-3 grid-cols-12 border-b-[1px] border-secondary'>
        ...
        <div id='like-comment-section' className='flex col-span-4'>
          {isliked ? (<><FaHeart onClick={handleLike} className='size-8 m-2 text-red-600 animate-pop' /> <p>{likesCount}</p></>) : (<FaRegHeart onClick={handleLike} className='size-8 m-2' />)}
